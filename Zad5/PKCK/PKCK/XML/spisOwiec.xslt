            ...
            <xsl:for-each select="//rasa[@idr=$rasa]">
              ...
              <pastwisko nazwa="{$nazwaPastwiska}">
                <powierzchnia jednostka="ha">
                  <xsl:if test="../../powierzchnia[@jednostka='ha']">
                    <xsl:value-of select="../../powierzchnia"/>
                  </xsl:if>
                  <xsl:if test="../../powierzchnia[@jednostka='ar']">
                    <xsl:value-of select="format-number(../../powierzchnia * 0.01,'##.0')"/>
                  </xsl:if>
                  <xsl:if test="../../powierzchnia[@jednostka='m2']">
                    <xsl:value-of select="format-number(../../powierzchnia * 0.0001,'##.0')"/>
                  </xsl:if>
                </powierzchnia>
                <stado>
                  <xsl:for-each select="../owca">
                    ...
                    <owca>
                      ...
                      <waga jednostka="kg">
                        <xsl:if test="waga[@jednostka='kg']">
                          <xsl:value-of select="waga"/>
                        </xsl:if>
                        <xsl:if test="waga[@jednostka='funt']">
                          <xsl:value-of select="format-number(waga * 0.4,'##.0')"/>
                        </xsl:if>
                        <xsl:if test="waga[@jednostka='dkg']">
                          <xsl:value-of select="format-number(waga * 0.01,'##.0')"/>
                        </xsl:if>
                      </waga>
                      <długość_runa jednostka="mm">
                        <xsl:if test="długość_runa[@jednostka='mm']">
                          <xsl:value-of select="długość_runa"/>
                        </xsl:if>
                        <xsl:if test="długość_runa[@jednostka='cm']">
                          <xsl:value-of select="format-number(długość_runa * 10,'##.0')"/>
                        </xsl:if>
                        <xsl:if test="długość_runa[@jednostka='cal']">
                          <xsl:value-of select="format-number(długość_runa * 25.4,'##.0')"/>
                        </xsl:if>
                      </długość_runa>
                      <szybkość_marszu jednostka="ms">
                        <xsl:if test="szybkość_marszu[@jednostka='ms']">
                          <xsl:value-of select="szybkość_marszu"/>
                        </xsl:if>
                        <xsl:if test="szybkość_marszu[@jednostka='kms']">
                          <xsl:value-of select="format-number(szybkość_marszu * 1000,'##.0')"/>
                        </xsl:if>
                        <xsl:if test="szybkość_marszu[@jednostka='kmh']">
                          <xsl:value-of select="format-number(szybkość_marszu * 0.28,'##.0')"/>
                        </xsl:if>
                      </szybkość_marszu>
                      <szybkość_zjadania_trawy jednostka="talerzmin">
                        <xsl:if test="szybkość_zjadania_trawy[@jednostka='talerzmin']">
                          <xsl:value-of select="szybkość_zjadania_trawy"/>
                        </xsl:if>
                        <xsl:if test="szybkość_zjadania_trawy[@jednostka='miskamin']">
                          <xsl:value-of select="format-number(szybkość_zjadania_trawy * 2,'##.0')"/>
                        </xsl:if>
                        <xsl:if test="szybkość_zjadania_trawy[@jednostka='wiadroh']">
                          <xsl:value-of select="format-number(szybkość_zjadania_trawy * 3,'##.0')"/>
                        </xsl:if>
